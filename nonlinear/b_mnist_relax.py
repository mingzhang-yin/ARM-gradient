...
def nonlinear_layer(x, num_latents, name, reuse):
    with tf.variable_scope(name, reuse=reuse):
        h1 = tf.layers.dense(2. * x - 1., num_latents, activation=tf.nn.relu, name="h1")
        h2 = tf.layers.dense(h1, num_latents, activation=tf.nn.relu, name="h2")
...
#    iwae_elbo = -(tf.reduce_logsumexp(-f_b) - np.log(valid_batch_size))
    ...
    def get_loss(sess,data,total_batch,batch_size,f_b):
        cost_eval = []                  
    ...
    EXPERIMENT = 'MNIST_Bernoulli_' + 'REBAR' if relaxation == 'rebar' else 'MNIST_Bernoulli_' + 'RELAX'
    ...
    for epoch in range(5000):
        ...
        if epoch%1 == 0:
            valid_loss = get_loss(sess,valid_data,total_valid_batch,batch_size,f_b)
        ...
        if epoch%5 == 0:
            COST_TEST.append(get_loss(sess,test_data,total_test_batch,batch_size,f_b))  
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="NL1") #NL1,L2
    parser.add_argument("--max_iters", type=int, default="800000") #800000 == 360epoch ==3.7h
