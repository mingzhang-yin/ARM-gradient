...
def nonlinear_layer(x, num_latents, name, reuse):
    with tf.variable_scope(name, reuse=reuse):
        h1 = tf.layers.dense(2. * x - 1., num_latents, activation=tf.nn.tanh, name="h1")
        h2 = tf.layers.dense(h1, num_latents, activation=tf.nn.tanh, name="h2")
...
#    iwae_elbo = -(tf.reduce_logsumexp(-f_b) - np.log(valid_batch_size))
    ...
    EXPERIMENT = 'layer2_MNIST_Bernoulli_' + 'REBAR' if relaxation == 'rebar' else 'layer2_MNIST_Bernoulli_' + 'RELAX'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="L2") #NL1,L2
    parser.add_argument("--max_iters", type=int, default="5000000")
