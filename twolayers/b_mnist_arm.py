...
def encoder1(x,bi_dim,reuse=False):
    ...
    with tf.variable_scope("encoder") as scope:
        ...
        log_alpha1 = tf.layers.dense(2. * x - 1., bi_dim, name="encoder_1")
    return log_alpha1
...
def encoder2(b1,bi_dim,reuse=False):
    ...
    with tf.variable_scope("encoder") as scope:
        if reuse:
            scope.reuse_variables()
        log_alpha2 = tf.layers.dense(2. * b1 - 1., bi_dim, name="encoder_2")
    return log_alpha2
...
def decoder1(b1,x_dim,reuse=False):
    ...
    with tf.variable_scope("decoder") as scope:
        if reuse:
            scope.reuse_variables()
        log_alphax = tf.layers.dense(2. * b1 - 1., x_dim, None, name="decoder_1")
    return log_alphax
...
def decoder2(b2,bi_dim,reuse=False):
    ...
    with tf.variable_scope("decoder") as scope:
        if reuse:
            scope.reuse_variables()
        log_alphab1 = tf.layers.dense(2. * b2 - 1., bi_dim, None, name="decoder_2")
    return log_alphab1




def fun(x_star,E1,E2,axis_dim=1,reuse_encoder=False,reuse_decoder=False):
    '''
    ...
    #log p(E)
    log_p_b = bernoulli_loglikelihood(E2, tf.zeros_like(E2)) 
...
gen_loss0 = fun(x_binary,b_sample1,b_sample2,reuse_encoder=True,reuse_decoder= False)
...
F2_1 = fun(x_binary,b_sample1,E2_1,reuse_encoder=True,reuse_decoder=True) #N,
F2_2 = fun(x_binary,b_sample1,E2_2,reuse_encoder=True,reuse_decoder=True)
...
F1_1 = fun(x_binary,E1_1,b_sample2_1,reuse_encoder=True,reuse_decoder=True) #N,
...
F1_2 = fun(x_binary,E1_2,b_sample2_2,reuse_encoder=True,reuse_decoder=True) #N,
...
EXPERIMENT = 'layer2_MNIST_Bernoulli_ARM'
