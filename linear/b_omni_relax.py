...
def Q_func(x, x_mean, z, bs, name, reuse, depth):
    ...
    with tf.variable_scope(name, reuse=reuse):
        h1 = tf.layers.dense(inp, 200, tf.nn.tanh, name="1")
        h2 = tf.layers.dense(h1, 200, tf.nn.tanh, name="2")
        if depth == 2:
...
#    total_test_batch = int(X_te.shape[0] / batch_size)
    ...
    EXPERIMENT = 'Linear_OMNI_Bernoulli_' + 'REBAR' if relaxation == 'rebar' else 'Linear_OMNI_Bernoulli_' + 'RELAX'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_iters", type=int, default="5000000") #500000 == 542epoch==2.5h
